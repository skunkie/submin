...
function reloadX(response, X, Xplural, Xcapital) {
	...
	for (var i = 0; i < Xs.length; ++i) {
		...
		if (status == "ok") {
			...
			if (status == "encoding error") {
				nameNode.setAttribute("title", "Encoding of repository name might give problems, please rename");
				nameNode.setAttribute("class", "err_encoding");
			}
